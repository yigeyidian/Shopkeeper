...
public class OrderManageDetailPresenter extends BasePresenter<IOrderManageDetailView> {

    public OrderManageDetailPresenter(Context context, IOrderManageDetailView iView) {
        super(context, iView);
    }
...
//                        iView.success(Arrays.asList(beens));
    ...
    public void getOrderManageDetail( int page, String billId) {
        DialogUtils.showDialog(context, "数据加载中");
        RetrofitHelper.getInstance()
                .getApi()
                .getOrderManageDetail("2", "ASC", 20, page, billId)
                .compose(getActivityLifecycleProvider().bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(stringModel -> {

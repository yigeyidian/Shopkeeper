...
public class DeskTypePresenter extends BasePresenter<IDeskTypeView> {

    public DeskTypePresenter(Context context, IDeskTypeView iView) {
        super(context, iView);
    }
    ...
    public void delete(DeskTypeBean bean) {
        DialogUtils.showDialog(context, "数据提交中");
        RetrofitHelper.getInstance()
                .getApi()
                .deleteDesktype("7", bean.getGuid(), App.INSTANCE().getShopID())
                .compose(getActivityLifecycleProvider().bindToLifecycle())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(stringModel -> {
